...
pub fn run(cmd: ScheduleCmd) -> Result<()> {
    match cmd {
        ScheduleCmd::Add { title, at, r#type, location, description } => {
            ...
            let id = brain::schedule_insert(
                &conn, &user_id, &title, &r#type, &start_at,
                location.as_deref(), description.as_deref(),
            )?;
            println!("{} {} @ {} ({})", style("✓").green(), style(&title).bold(), style(&start_at).cyan(), style(&id).dim());
            Ok(())
        }
        ScheduleCmd::List { all } => {
            ...
            println!("{:<20}  {:<10}  {:<20}  {}",
                style("일시").bold(), style("타입").bold(), style("장소").bold(), style("제목").bold());
            println!("{}", "-".repeat(70));
            for s in schedules {
                let location = s.location.unwrap_or_default();
                let start = s.start_at.get(..16).unwrap_or(&s.start_at).to_string();
                println!("{:<20}  {:<10}  {:<20}  {}", start, s.r#type, location, s.title);
            }
            Ok(())
        }
    }
